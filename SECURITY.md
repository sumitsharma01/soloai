...
Known limitations: one owner per workspace; no verified email, password recovery, MFA or invitation workflow; no RLS; runtime database bootstrap privilege must be reduced; no live inbox OAuth; legacy Bicep has no model-private-endpoint or WAF provisioning; the current Terraform target adds private AI access and Front Door WAF but has not been deployed/penetration-tested; no durable worker/cancellation of already-started model inference; no per-end-user quotas on the customer's side. The customer server must authenticate and rate-limit its users. A server API key must never be exposed in a public chat widget. Signup token grants are not abuse-resistant until verified identity and edge admission controls are added.
